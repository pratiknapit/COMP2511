...
int TreeHeight(Tree t)
{
    ...
    else if (t->left == NULL && t->right == NULL)
    {
        ...
        //int max = (lh > rh) ? lh : rh; - use this turner if statement 
...
int TreeNumNodes(Tree t)
{
    if (t == NULL) {
        ...
        int lc = TreeNumNodes(t->left); //Left tree
        int rc = TreeNumNodes(t->right);    //Right tree
        
        int ans = lc + rc + 1; //add current node with left count and right count
        return ans;
    }
