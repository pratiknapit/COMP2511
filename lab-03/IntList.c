...
void IntListInsertInOrder(IntList l, int v) {
	assert(l != NULL); //empty list (case 0)
	...
	//if we need to insert at the beginning of the list for the smallest value, we insert after list->first
	if (l->first == curr) { // insert as the first element (case 1)
		n->next = curr;
		l->first = n; 
	} else if (curr == NULL) { //insert as last element, same as appending 
		l->last->next = n;
		l->last = n;
...
IntList IntListSortedCopy(IntList l) {
	...
	// create a new list - need to memory allocate as well.
	// then we just need to find the minimum in the list and then add it to the new list. 

	//alternatively, we can just start with a number. And then add the new number using the IntListInsertInOrder

	IntList NewList = IntListNew();
	struct IntListNode *curr = l->first;
	
	IntListAppend(NewList, curr->data);
	curr = curr->next;
	//NewList->size++;
	
	while(curr != NULL) {
		//need to add this data to NewList 
		IntListInsertInOrder(NewList, curr->data);
		curr = curr->next;
		//NewList->size++; 
	}
